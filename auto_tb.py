...
def writeTestBench(input_file):
    ...
    tb_full_name = os.path.join(dir_name, 'tb_' + file_name)
    tb = open(tb_full_name, "w")
    ...
    print("//~ `New testbench", file=tb)
    print(timescale, file=tb)
    print("module tb_%s;\n" % name, file=tb)

    # module_parameter_port_list
    if(paraDec!=''):
        print("// %s Parameters\n%s\n" % (name, paraDec), file=tb)
...
'''
    ...
    # UUT
    print("%s %s u_%s (\n%s\n);" %(name,paraDef,name,portList), file=tb)
...
'''
    print(operation, file=tb)
    print("endmodule", file=tb)
